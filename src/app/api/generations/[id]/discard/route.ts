...
export async function POST(
  ...
  // would lose an increment if a brand has two generations discarded in
  // quick succession. credit_transactions uses credits/balance_after,
  // matching every other live ledger write in the codebase.
  if (collabSessionId) {
    try {
      ...
      if (creditUpd) {
        ...
        console.error(
          `[discard] credit refund conflict for gen=${id}, manual reconcile needed`,
        );
      }
    } catch (err) {
