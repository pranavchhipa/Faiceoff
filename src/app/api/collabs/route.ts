...
export async function GET() {
  ...
  // brand + creator lookups are independent → run together
  ...
  // Three independent enrichment chains run in PARALLEL:
  //   (1) counterpart names/avatars  (2) product images  (3) brand pending payments
  let pendingPayments: unknown[] = [];

  await Promise.all([
    // (1) Counterpart display names + avatars
    (async () => {
    ...
    // (2) Product images via linked collab_requests (one query)
    (async () => {
      ...
      const { data: reqs } = await admin
        ...
        .in("id", requestIds);
      for (const r of reqs ?? []) productImageMap[r.id] = r.product_image_url ?? null;
    })(),

    // (3) Brand-only: accepted requests awaiting payment + creator names
    (async () => {
      if (!brand) return;
