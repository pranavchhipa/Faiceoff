...
export async function POST(
  ...
  // Brand's typed electronic signature on the Collaboration Agreement, captured
  // on the payment screen before checkout. Null on the webhook path (we fall
  // back to the company name as an auto-recorded signatory below).
  let brandSignedName: string | null = null;
  ...
  // ── Collaboration Agreement: brand signs + agreement activates ──
  // The creator already signed at accept (status pending_brand). Recording the
  // brand's signature here flips it to 'active'. We do the fast sign+activate
  // inline so the collab page shows a signed agreement immediately, then render
  // the dual-signed PDF in after() (PDF rendering is slow — must not block).
  try {
    let effectiveSigner: string = brandSignedName ?? "";
    if (!effectiveSigner) {
      // Webhook / reconciliation path — no typed name. Record the company as an
      // auto-signatory (the brand consented by completing payment).
      const { data: b } = await admin
        .from("brands").select("company_name").eq("id", req.brand_id).maybeSingle();
      effectiveSigner = b?.company_name ?? "Authorized signatory";
    }
    const agreementRow = await signBrandAndActivate({
      admin,
      requestId,
      sessionId: session.id,
      brandSignedName: effectiveSigner,
      brandSignedIp: clientIpFromHeaders(request.headers),
    });
    if (agreementRow && !(agreementRow.status === "active" && agreementRow.pdf_url)) {
      after(async () => {
        try {
          ...
          console.error("[confirm-payment] agreement PDF render failed", err);
        }
        // Email + in-app notify BOTH sides the agreement is signed & active.
        await notifyAgreementActivated(admin, agreementRow);
      });
    }
  } catch (err) {
    // Non-fatal — payment + session already succeeded. Agreement can be
    // reconciled/regenerated by an admin.
    console.error("[confirm-payment] agreement finalize failed (non-fatal)", err);
  }
